...
import {OcorrenciaStatusGravacaoService} from '../../../app/ocorrencias/ocorrencia-status-gravacao.service';
...
export class EventoEffects {
  constructor(
    ...
    private _ocorrenciaStatusGravacaoService: OcorrenciaStatusGravacaoService,
  ...
  obtemEventos$ = this._actions$.pipe(
    ...
    switchMap((action: fromActions.ObtemEventosPaginadosRun) =>
      this._eventoService
        .findEventosByUsuarioLogado(
          action.payload.termoDeBusca,
          action.payload.paginacao && action.payload.paginacao.page
            ? action.payload.paginacao.page
            : 0,
          action.payload.paginacao && action.payload.paginacao.pageSize
            ? action.payload.paginacao.pageSize
            : 10,
        )
        .pipe(catchError(() => observableOf({content: [], totalElements: 0}))),
    ),
    map(
      (
        pagina:
  ...
  obtemTiposEventos$ = this._actions$.pipe(
    ...
    map((tiposEventos: TipoEvento[]) => new fromActions.ObtemTiposDeEventosSuccess({tiposEventos})),
  );
  ...
  obtemEventoById$ = this._actions$.pipe(
    ...
    switchMap((action: fromActions.ObtemEventoPorIdRun) =>
      this._eventoService
        .findEventoById(action.payload.id)
        .pipe(catchError((e) => observableOf(e))),
    ),
    map((evento: Evento | string) =>
      typeof evento === 'string'
        ? new fromActions.ObtemEventoPorIdFail({error: '', eventoId: evento})
  ...
  alteraTipoEvento$ = this._actions$.pipe(
    ...
    map((evento: Evento) => new fromActions.AlteraTipoDeEventoSuccess({evento})),
  );
  ...
  reabreEvento$ = this._actions$.pipe(
    ...
    switchMap((action: fromActions.ReabreEventoRun) =>
      this._eventoService.reabreEvento(
        action.payload.eventoId,
        action.payload.textoComentario ? action.payload.textoComentario : '',
      ),
    ),
    tap((evento: Evento) =>
  ...
  insereComentario$ = this._actions$.pipe(
    ...
    switchMap((action: fromActions.InsereComentarioRun) =>
      this._eventoService
        .insereComentario(
          action.payload.eventoId,
  ...
  criaEvento$ = this._actions$.pipe(
    ...
    switchMap((action: fromActions.CriaEventoRun) =>
      this._eventoService
        .criaEvento(action.payload.novoEventoRequest)
        .pipe(catchError((error: NovoEventoRequest) => observableOf(error))),
    ),
  ...
  alteraLocalEvento$ = this._actions$.pipe(
    ...
    switchMap((action: fromActions.AlteraLocalDoEventoRun) =>
      this._eventoService
        .alteraLocalDoEvento(action.payload.eventoId, action.payload.local)
    ...
    map((evento: Evento) => new fromActions.AlteraLocalDoEventoSuccess({evento})),
  );

  @Effect()
  alteraUnidadeEvento$ = this._actions$.pipe(
    ofType(fromActions.ALTERA_UNIDADE_DO_EVENTO.RUN),
    switchMap((action: fromActions.AlteraUnidadeDoEventoRun) =>
      this._eventoService
        .alteraUnidadeDoEvento(action.payload.eventoId, action.payload.unidade)
        .pipe(catchError((e) => observableOf('erro'))),
    ),
    tap((item: Evento | string) =>
      typeof item === 'string'
    ...
    map((evento: Evento) => new fromActions.AlteraUnidadeDoEventoSuccess({evento})),
  );

  @Effect()
  alteraTituloEvento$ = this._actions$.pipe(
    ofType(fromActions.ALTERA_TITULO_DO_EVENTO.RUN),
    switchMap((action: fromActions.AlteraTituloDoEventoRun) =>
      this._eventoService
        .alteraTituloDoEvento(action.payload.eventoId, action.payload.titulo)
        .pipe(catchError((e) => observableOf('erro'))),
    ),
    tap((item: Evento | string) =>
      typeof item === 'string'
    ...
    map((evento: Evento) => new fromActions.AlteraTituloDoEventoSuccess({evento})),
  );
  ...
  alteraVisibilidadeDaInteracao$ = this._actions$.pipe(
    ...
    tap((item: Evento | string) =>
      typeof item === 'string'
        ? this._ocorrenciaStatusGravacaoService.statusGravacao$.next({
            sucesso: false,
            operacaoExecutada: OcorrenciaChangeType.VISIBILIDADE_COMENTARIO,
          })
        : this._ocorrenciaStatusGravacaoService.statusGravacao$.next({
            sucesso: true,
            operacaoExecutada: OcorrenciaChangeType.VISIBILIDADE_COMENTARIO,
    ...
    map((evento: Evento) => new fromActions.AlteraVisibilidadeDaInteracaoSuccess({evento})),
  );
  ...
  alteraVisibilidadeDoEvento$ = this._actions$.pipe(
    ...
    switchMap((action: fromActions.AlteraVisibilidadeDoEventoRun) =>
      this._eventoService
        .alteraVisibilidadeDoEvento(action.payload.eventoId, action.payload.visibilidade)
        .pipe(catchError((e) => observableOf('erro'))),
    ),
    tap((item: Evento | string) =>
      typeof item === 'string'
  ...
  alteraTextoDeComentario$ = this._actions$.pipe(
    ...
    map((evento: Evento) => new fromActions.AlteraTextoDeComentarioSuccess({evento})),
  );
  ...
  alteraParecerDoEvento$ = this._actions$.pipe(
    ...
    switchMap((action: fromActions.AlteraParecerDoEventoRun) =>
      this._eventoService
        .alteraParecerDoEvento(action.payload.eventoId, action.payload.textoFormatado)
        .pipe(catchError((e) => observableOf('erro'))),
    ),
    tap((item: Evento | string) =>
      typeof item === 'string'
    ...
    map((evento: Evento) => new fromActions.AlteraParecerDoEventoSuccess({evento})),
  );

  @Effect()
  alteraParticipantesDoEvento$ = this._actions$.pipe(
    ofType(fromActions.ALTERA_PARTICIPANTES_DO_EVENTO.RUN),
    switchMap((action: fromActions.AlteraParticipantesDoEventoRun) =>
      this._eventoService
    ...
    tap((item: Evento | string) =>
      typeof item === 'string'
        ? this._ocorrenciaStatusGravacaoService.statusGravacao$.next({
            sucesso: false,
            operacaoExecutada: OcorrenciaChangeType.ALTERA_PARTICIPANTES,
          })
        : this._ocorrenciaStatusGravacaoService.statusGravacao$.next({
            sucesso: true,
            operacaoExecutada: OcorrenciaChangeType.ALTERA_PARTICIPANTES,
    ...
    map((evento: Evento) => new fromActions.AlteraParticipantesDoEventoSuccess({evento})),
  );

  @Effect()
  alterarResponsaveisDoEvento$ = this._actions$.pipe(
    ofType(fromActions.ALTERA_RESPONSAVEIS_DO_EVENTO.RUN),
    switchMap((action: fromActions.AlteraResponsaveisDoEventoRun) =>
      this._eventoService
    ...
    map((evento: Evento) => new fromActions.AlteraResponsaveisDoEventoSuccess({evento})),
  );
}
