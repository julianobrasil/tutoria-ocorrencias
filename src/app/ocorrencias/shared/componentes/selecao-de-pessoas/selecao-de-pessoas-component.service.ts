...
export class SelecaoDePessoasComponentService {
   ...
   */
  _adicionaPessoasSeNaoExistir(
    pessoas: ObjectReference[],
    pessoasEscolhidas: ObjectReference[],
  ): void {
    pessoasEscolhidas.forEach((pe: ObjectReference) => {
      const index = pessoas.findIndex((p: ObjectReference) => pe.code === p.code);
      if (index > -1) {
        pessoas.splice(index, 1);
      }
    });

    pessoasEscolhidas.sort((a: ObjectReference, b: ObjectReference) =>
