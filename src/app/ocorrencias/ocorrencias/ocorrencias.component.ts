...
import {OcorrenciaStoreFacadeService} from '../ocorrencia-facade.service';
import {
...
import {ListaDeOcorrenciasSearchBarComponentChange} from '../lista-de-ocorrencias-search-bar/lista-de-ocorrencias-search-bar.component';
...
export class OcorrenciasComponent implements OnDestroy {
  ...
  /** dados de paginação */
  _paginacao$: Observable<IssueTrackerPagination> = this._ocorrenciaStoreFacade.getPaginacao$();
  ...
  _diferencaDeHoraNoServidor$ = this._ocorrenciaStoreFacade.getDiferencaDeHoraNoServidor$();

  /** filtros disponíveis */
  _filtrosDeBusca$ = this._ocorrenciaStoreFacade.getFiltrosDeBusca$();

  /** unidades disponíveis */
  _unidades$: Observable<Unidade[]> = this._ocorrenciaStoreFacade.getUnidades$();
  ...
  _mostraFormularioDeOcorrencia(classificacao: OcorrenciaFormularioComponentTipo) {
    this._ocorrenciaFormularioComponentTipo = classificacao;

    this._router.navigate(['nova-ocorrencia', this._ocorrenciaFormularioComponentTipo], {
  ...
  /** Atualiza busca porque houve uma alteração no filtro */
  _trataAlteracaoNoFiltro(change: ListaDeOcorrenciasSearchBarComponentChange) {
    this._ocorrenciaStoreFacade.setFiltroDeBuscaEBusca(
      Object.assign(new FiltrosDeBusca(), {...change}),
    );
  }
  ...
  private _obtemOcorrenciaAPartirDaRota() {
    this._activatedRoute.paramMap
      .pipe(
        map((p: ParamMap) => (p.get('tipoNovaOcorrencia') ? p.get('tipoNovaOcorrencia') : null)),
