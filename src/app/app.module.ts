...
export class AppModule {
  constructor(private _store$: Store<{}>) {
    this._fixEventos();
    this._store$.dispatch(new fromStore.GERAL.ACTIONS.ObtemDadosDoUsuarioLogadoRun());
    this._store$.dispatch(new fromStore.GERAL.ACTIONS.ObtemTutoriasDoUsuarioLogadoRun());
    this._store$.dispatch(new fromStore.GERAL.ACTIONS.ObtemConfigRun());
    this._store$.dispatch(new fromStore.GERAL.ACTIONS.ObtemUnidadesDisponiveisRun());
    this._store$
      .pipe(
        select(fromStore.GERAL.SELECTORS.CONFIGURACOES_GERAIS.getSemestreDeTrabalho),
        filter(Boolean),
        first(),
        tap((s: fromDocuments.SemestreDeTrabalho) =>
          this._store$.dispatch(
            new fromStore.GERAL.ACTIONS.ObtemDatasDeEntregaDisponiveisRun({
              ano: s.ano,
              semestre: s.semestre,
            }),
          ),
  ...
  _fixEventos(): void {
    EVENTOS_EXISTENTES.forEach((evt: fromDocuments.Evento) => {
      ...
      evt.cidadeUnidade = `${evt.tutoria.unidade.cidade}:${evt.tutoria.unidade.unidade}`;
      ...
      evt.interacoes = evt.etapas
        .map(
          (etapa: fromDocuments.Etapa): fromDocuments.Interacao => ({
        ...
        const acao: fromDocuments.Interacao = {
          ...
          autorRef:
            evt.interacoes && evt.interacoes.length
              ...
              : {
                  code: evt.tutoria.historicoTutores.find((t) => !t.dataFim).email,
                  description: evt.tutoria.historicoTutores.find((t) => !t.dataFim).nomeTutor,
                },
          id: '' + new Date().getTime(),
          role:
            evt.interacoes && evt.interacoes.length
      ...
      evt.participantes = evt.etapas.map((etapa: fromDocuments.Etapa) => ({
        usuarioRef: {
          code: etapa.emailResponsavel,
          description: etapa.emailResponsavel,
        },
