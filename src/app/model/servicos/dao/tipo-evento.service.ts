...
export class TipoEventoService {
  ...
  saveTipoEvento(tipo: TipoEvento): Observable<TipoEvento[]> {
    return this.http
      .post(URL.baseTutoriaServiceURL + URL.tipoEventoSalvarPath, JSON.stringify(tipo), {
        observe: 'response',
      })
      .pipe(
  ...
  findTipoEventoLikeEmail(
    email: string,
    cidade?: string,
    unidade?: string,
  ): Observable<TipoEvento[]> {
    ...
    return this.http
      .get(URL.baseTutoriaServiceURL + URL.tipoEventoByEmailPath, {observe: 'response', params})
      .pipe(
        map((res: HttpResponse<any>) => {
          if (res.ok) {
            return res.body as TipoEvento[];
  ...
  removeTipoEvento(id: string): Observable<boolean> {
    return this.http
      .delete(URL.baseTutoriaServiceURL + URL.tipoEventoRemoverPath + id, {observe: 'response'})
      .pipe(
        map((res: HttpResponse<any>) => {
          if (res.ok) {
            return true;
