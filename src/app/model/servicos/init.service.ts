...
export class InitService {
  ...
  getUserAuthorities(): Observable<UserData | null> {
    return this._http.get<any>(URL.baseUserServiceURL + URL.usuarioLogadoPath)
        .pipe(map((dadosDeUsuario) => {
            ...
            if (authorityItem['authority']) {
              if (authorityItem['authority'].indexOf('completeName=') > -1) {
                userData.nomeUsuario =
                    authorityItem['authority'].split('completeName=')[1];
              }

              if (authorityItem['authority'].indexOf('givenName=') > -1) {
                userData.apelido =
                    authorityItem['authority'].split('giveName=')[1];
              }
              ...
              if (authorityItem['authority'].indexOf('appID=') > -1 &&
                ...
                // alvo: sp = [XXXXXX,
                // YYYYYYYYYappUri=:::sgi:::sgiservice:::]
  ...
  getUserPermissions(): Observable<string | null> {
    return this._http.get<any>(URL.baseTutoriaServiceURL +
                               URL.permissoesUsuarioAutenticadoPath)
        .pipe(map((res) => {
          const userPermissionsList = res.reduce(
              (a: string, b: string) => a + (a !== '' ? ',' : '') + b, '');
  ...
  obtemDataEHora() {
    return setInterval(() => this._obtemDataEHoraSubcriber(),
                       120000 + Math.random() * 120000);
  }

  private _obtemDataEHoraLogic(): Observable<any> {
    return this._http.get<any>(URL.baseTutoriaServiceURL +
                               URL.dataEHoraServicePath)
        .pipe(retry(20), catchError((err) => observableOf(null)));
  }
  ...
  obtemDemaisDadosDaSessao(userData: UserData,
                           userPermissionsList: string): Observable<any> {
    // obtendo data e hora
    console.log('loading data e hora');
    const dataEHoraObservable = this._obtemDataEHoraLogic().pipe(map((obj) => {
    ...
    const dadosDeUsuarioLogadoObservable =
        this._verificaSeASessaoEDoUsuarioCorrente(userData.email);
    ...
    if (userPermissionsList.indexOf(Permissoes.ADMINISTRACAO_SISTEMA) > -1 ||
        userPermissionsList.indexOf(Permissoes.CADASTRO_USUARIO_CONSULTAR) >
            -1 ||
      ...
      forkJoin(unidadeObservable, usuariosObservable, permissoesObservable,
               funcoesObservable)
          .pipe(retry(1))
          .subscribe();
    }

    return forkJoin(dadosDeUsuarioLogadoObservable, configObservable,
                    tipoEventoObservable, dataEHoraObservable)
        .pipe(retry(1));
  }
  ...
  private _obtemUsuarios(): Observable<UsuarioAD[] | null> {
    return this._http.get<UsuarioAD[]>(URL.baseUserServiceURL +
                                       URL.usuarioLdapTodosPath)
        .pipe(tap((usuarios: UsuarioAD[]) => {
  ...
  private _obtemPermissoesDisponiveis(): Observable<Permissao[] | null> {
    return this._http.get<Permissao[]>(URL.baseTutoriaServiceURL +
                                       URL.permissoesTodasPath)
        .pipe(
  ...
  private _obtemFuncoesDisponiveis(): Observable<Funcao[] | null> {
    return this._http.get<Funcao[]>(URL.baseTutoriaServiceURL +
                                    URL.funcoesTodasPath)
        .pipe(tap((funcoes: Funcao[]) => {
                console.log('available funcions loaded');
                this._imodb.funcoes = funcoes;
                this._imodb.funcoes.sort(
                    (a: Funcao, b: Funcao) =>
                        a.nomeAmigavelFuncao.toUpperCase() ===
                                b.nomeAmigavelFuncao.toUpperCase() ?
                            0 :
                            a.nomeAmigavelFuncao.toUpperCase() <
                                    b.nomeAmigavelFuncao.toUpperCase() ?
                            -1 :
  ...
  private _obtemTiposDeEventosDisponiveis(): Observable<TipoEvento[] | null> {
    return this._http.get<TipoEvento[]>(URL.baseTutoriaServiceURL +
                                        URL.tipoEventoTodosPath)
        .pipe(tap((tipoEventos: TipoEvento[]) => {
  ...
  private _obtemConfiguracoesExistentes(): Observable<string | null> {
    return this._http.get<Config>(URL.baseTutoriaServiceURL +
                                  URL.configUnicaPath)
        .pipe(map((config: Config) => {
              ...
              switchMap(() => {
                ...
                dataDeEntrega.semestreDeTrabalho =
                    this._imodb.config.semestreDeTrabalho;
